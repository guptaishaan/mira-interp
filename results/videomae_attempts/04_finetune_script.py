...
def pilot(report, path):
    ...
    if len(gradients) != len(list(tail.parameters())): raise ValueError('Not all selected parameters receive gradients')
    tail.zero_grad(set_to_none=True)
    with torch.autocast('cuda', dtype=torch.bfloat16):
        loss_cached = torch.mean((tail(prefix.detach().cpu().float().cuda())-target)**2)
    loss_cached.backward()
    gradient_error = max(float((p.grad-gradients[name]).abs().max()) for name, p in tail.named_parameters())
    cached_gradients = {name: p.grad.detach().clone() for name, p in tail.named_parameters()}
    tail.zero_grad(set_to_none=True)
    with torch.autocast('cuda', dtype=torch.bfloat16):
    ...
    save(path, report)
    if any(not torch.allclose(cached_gradients[name], gradients[name], rtol=1e-5, atol=1e-5) for name, p in tail.named_parameters()):
