...
from mira_interp.video_evaluator_finetune import VideoMAETail, frozen_prefix, optimizer_for, parameter_digest, deterministic_pool
...
def pilot(report, path):
    ...
    with torch.autocast('cuda', dtype=torch.bfloat16):
        ...
        full_hidden = encoder(pixel_values=video).last_hidden_state
        legacy_descriptor = pool_last_tubelet(full_hidden)
        full_descriptor = deterministic_pool(full_hidden)
        cached_descriptor = tail.descriptor(prefix.detach().cpu().float().cuda())
    pool_error = float((full_descriptor-legacy_descriptor).abs().max())
    if not torch.allclose(full_descriptor, legacy_descriptor, rtol=1e-5, atol=1e-5):
        raise ValueError('Deterministic pooling does not reproduce the original adaptive bins')
    ...
    with torch.autocast('cuda', dtype=torch.bfloat16):
        full_prediction = tail.head(deterministic_pool(encoder(pixel_values=video).last_hidden_state))
    ...
    tail.zero_grad(set_to_none=True)
    with torch.autocast('cuda', dtype=torch.bfloat16):
        repeated_loss = torch.mean((tail.head(deterministic_pool(encoder(pixel_values=video).last_hidden_state))-target)**2)
    ...
    report.update(gradient_diagnostics=diagnostics, prefix_requires_grad=prefix.requires_grad,
        ...
        original_pool_forward_max_abs=pool_error,
